...
describe("Add key manager", () => {
    ...
    const addKeyManager = (usernameLocal, passwordLocal) => {
        ...
        cy.get('[data-testid="add-key-manager-button"]', { timeout: Cypress.config().largeTimeout }).should('be.visible');
        ...
        cy.wait('@importConfig', { timeout: 3000 }).then(() => {
            ...
            cy.get('td > div', { timeout: Cypress.config().largeTimeout }).contains(km).should('exist');
        });
